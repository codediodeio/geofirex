...
var C__Users_lenny_Desktop_geofirex_node_modules_rbush = rbush;
var default_1 = rbush;
...
C__Users_lenny_Desktop_geofirex_node_modules_rbush.default = default_1;
...
var C__Users_lenny_Desktop_geofirex_node_modules_monotoneConvexHull2d = monotoneConvexHull2D;
...
var C__Users_lenny_Desktop_geofirex_node_modules_tinyqueue = TinyQueue;
...
C__Users_lenny_Desktop_geofirex_node_modules_tinyqueue.default = default_1$1;

var C__Users_lenny_Desktop_geofirex_node_modules_pointInPolygon = function (point, vs) {
...
var C__Users_lenny_Desktop_geofirex_node_modules_concaveman = concaveman;
...
function concaveman(points, concavity, lengthThreshold) {
    ...
    var tree = C__Users_lenny_Desktop_geofirex_node_modules_rbush(16, ['[0]', '[1]', '[0]', '[1]']).load(points);
    ...
    var segTree = C__Users_lenny_Desktop_geofirex_node_modules_rbush(16);
...
function findCandidate(tree, a, b, c, d, maxDist, segTree) {
    var queue = new C__Users_lenny_Desktop_geofirex_node_modules_tinyqueue(null, compareDist);
    var node = tree.data;
...
function fastConvexHull(points) {
    ...
    for (i = 0; i < points.length; i++) {
        if (!C__Users_lenny_Desktop_geofirex_node_modules_pointInPolygon(points[i], cull)) filtered.push(points[i]);
    }

    // get convex hull around the filtered points
    var indices = C__Users_lenny_Desktop_geofirex_node_modules_monotoneConvexHull2d(filtered);
...
C__Users_lenny_Desktop_geofirex_node_modules_concaveman.default = default_1$2;
...
function convex(geojson, options) {
    ...
    var convexHull = C__Users_lenny_Desktop_geofirex_node_modules_concaveman(points, concavity);
...
var compute_geohash_tiles_from_polygon = function (polygon, precision) {
    var checked_geohashes = [];
    var geohash_stack = [];
    var geohashes = [];
    // get center of polygon, assuming the earth is flat
    var centerOfPoly = centerOfMass(polygon);
    var center_latitude = centerOfPoly.geometry.coordinates[1];
    var center_longitude = centerOfPoly.geometry.coordinates[0];
    var center_geohash = encode(center_latitude, center_longitude, precision);
    geohashes.push(center_geohash);
    geohash_stack.push(center_geohash);
    checked_geohashes.push(center_geohash);
    var _loop_1 = function () {
        var current_geohash = geohash_stack.pop();
        var area = neighbors(current_geohash);
        area.forEach(function (neighbor) {
            var lonlat = decode(current_geohash);
            var lat = lonlat.latitude;
            var lon = lonlat.longitude;
            var point = [lon, lat];
            if (checked_geohashes.indexOf(neighbor) === -1 && booleanPointInPolygon(point, polygon)) {
                geohashes.push(neighbor);
                if (geohash_stack.indexOf(neighbor) === -1)
...
var C__Users_lenny_Desktop_geofirex_node_modules__turf_distance = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", { value: true });
...
var distance = unwrapExports(C__Users_lenny_Desktop_geofirex_node_modules__turf_distance);
...
var GeoFireCollectionRef = /** @class */ (function () {
     ...
     */
    GeoFireCollectionRef.prototype.withinPolygon = function (polygon, precision, field) {
        var _this = this;
        precision = precision || 6;
        var area = compute_geohash_tiles_from_polygon(polygon, precision);
     ...
     */
    GeoFireCollectionRef.prototype.withinBbox = function (bbox, precision, field, opts) {
        var _this = this;
        var minlat = bbox[0], minlon = bbox[1], maxlat = bbox[2], maxlon = bbox[3];
        var area = bboxes(minlat, minlon, maxlat, maxlon, setPrecsion(precision));
...
export { GeoFireClient, GeoFireCollectionRef, GeoFirePoint, get, init, toGeoJSON };
